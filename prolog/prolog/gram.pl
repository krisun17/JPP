...
grammar(ex11, [prod('F', [['(', nt('L'), ')']]),           % jfirst
              prod('L', [[nt('E'),nt('L')], []]),
              prod('E', [[nt('F')], ['a']]) ]).

%przerabianie na gramatyke z epsilonami
toeps( [ prod( X, [ [] | L ]) | XP], [ prod( X, [ ['eps'] | L1 ]) | XS ]) :- toeps( [ prod( X, L ) | XP ], [ prod( X,  L1) | XS ]).
toeps( [ prod( X, [ L1 | L ]) | XP], [ prod( X, [ L1 | L2 ]) | XS ]) :- L1 \== [], toeps( [ prod( X, L ) | XP ], [ prod( X,  L2) | XS ]).
toeps( [ prod( X, []) | XP], [ prod( X, []) | XS ]) :- toeps( XP, XS).
toeps( [], [] ).

fromeps( [ prod( X, [ ['eps'] | L ]) | XP], [ prod( X, [ [] | L1 ]) | XS ]) :- fromeps( [ prod( X, L ) | XP ], [ prod( X,  L1) | XS ]).
fromeps( [ prod( X, [ L1 | L ]) | XP], [ prod( X, [ L1 | L2 ]) | XS ]) :- fromeps( [ prod( X, L ) | XP ], [ prod( X,  L2) | XS ]).
fromeps( [ prod( X, []) | XP], [ prod( X, []) | XS ]) :- fromeps( XP, XS).
fromeps( [], [] ).
...
jestPuste(A, prod(A, [['eps'] | L]) ).
jestPuste(A, prod(A, [X | L]) ) :- jestPuste(A, prod(A, L) ).

jestPusteG(A, []) :- !, fail.
jestPusteG(A, [P | XP]) :- jestPuste(A, P).
jestPusteG(A, [P | XP]) :- jestPusteG(A, XP).
...
first( prod(A, [ [X | Sym] | L]), [ X | WynA], G ) :- X \== 'eps', \+jestNt(G, X), first( prod(A, L), WynA, G ).
first( prod(A, [ ['eps'] | L]), WynA, G ) :- first( prod(A, L), WynA, G ).

firstL( A, G, Wyn) :- toeps(G, G1), wezProd(G1, A, P), first( P, Wyn, G1).

followOne( A, prod(X, []), [], G).
followOne( A, prod(X, [ [nt(A) | [nt(B) | Sym]] | L]), Wyn, G) :- A \== B, firstL(B, G, WynB), jestPusteG(B,G), followAll( B, G, WynX, G),
					followOne( A, prod(X, [ [nt(B) | Sym] | L]), WynA, G), append(WynX,WynA,Wyn1), append(Wyn1, WynB, Wyn).
followOne( A, prod(X, [ [nt(A) | [nt(B) | Sym]] | L]), Wyn, G) :- A \== B, firstL(B, G, WynB), followOne( A, prod(X, [ [nt(B) | Sym] | L]), WynA, G), append(WynA, WynB, Wyn).
followOne( A, prod(X, [ [nt(A)] | L]), Wyn, G) :- X \== A, followAll( X, G, WynX, G), followOne( A, prod(X, L), WynA, G), append(WynA, WynX, Wyn).
followOne( A, prod(A, [ [nt(A)] | L]), Wyn, G) :- followOne( A, prod(X, L), WynA, G), append(WynA, WynX, Wyn).
followOne( A, prod(X, [ [nt(A) | [B | Sym]] | L]), [ B | WynA ], G) :- \+jestNt(G,B), followOne( A, prod(X, [ [B | Sym] | L]), WynA, G).
followOne( A, prod(X, [ [B | Sym] | L]), Wyn, G) :- B \== nt(A), followOne( A, prod(X, [ Sym | L]), Wyn, G).
...
follow(G, Wyn) :- toeps(G,G1), listaNt(G1,L), followList(G1,L,Wyn).

%zbiory select

selectOne(A, [], [], G).
selectOne(A, [ 'eps' ], Wyn, G) :- followAll(A, G, Wyn, G).
selectOne(A, [ X | L ], [X], G) :- X \== 'eps', \+jestNt(G,X).
...
select(G,W) :- toeps(G,G1), selectAll(G1,W,G1).
...
jestLL21(G) :- toeps(G,G1), czyLL1(G1,G1).
